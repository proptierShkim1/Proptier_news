...
_NO_CONTEXT_NOTE = (
    " 이 질문과 관련된 뉴스·정책 원문 자료는 벡터 검색으로 찾지 못했어. 아래 지표 도구로 "
    "정확한 사내 통계(건수·추이·랭킹 등)를 조회해서 답할 수 있으면 그 결과를 근거로 답해줘 "
    "— 이 경우는 사내 데이터 기반 답변이니 일반 지식이라고 말하면 안 돼. 지표 도구로도 "
    "답할 수 없는 질문일 때만 일반적인 지식으로 답하고, 그때는 사내 데이터 기반 답변이 "
    "아니라는 점을 밝혀줘."
)
_WITH_CONTEXT_NOTE = (
