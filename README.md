...
| 📝 브리핑 | 날짜별 아침 브리핑 아카이브 — 달력 위젯으로 날짜 선택(확정/집계중 날짜를 점으로 표시), 채널별 수집 현황 + 카테고리별 TOP3만 표시(빈 섹션은 숨김). 확정된 과거 날짜는 채널 표시 설정이 나중에 바뀌어도 내용이 변하지 않는다 |
...
| 🤖 AI AGENT | Gemini 기반 자유 대화형 챗봇(`st.chat_input`) — 수집 데이터(뉴스·정책)를 벡터 검색으로 찾아 답변에 근거로 사용, 지표 성격 질문(건수·비용·순위 등)은 29개 함수 호출 도구로 직접 집계해 답변(그중 3개는 뉴스↔정책 카테고리를 엮는 온톨로지 기반 조회), 근거가 부족하면 opt-in Hybrid Search(웹 검색) 버튼 제공 |
| ⚙️ 설정 (관리자 전용) | 접근 제어(IP 화이트리스트) · 데이터 수집 · 데이터 관리 · 벡터 데이터(백업/복구 포함) · API 사용량 · 로그 · 서버 배포(`.env`에 배포 대상이 설정된 환경에서만 탭이 나타남, "서버 배포" 섹션 참고) |
...
- **날짜 선택 UI(2026-08-26)**: 아카이브가 쌓일수록 날짜 버튼 목록이 계속 길어지던 문제를
  `st.date_input` 달력 위젯으로 교체해 해결했다. 데이터가 없는 날짜는 min/max 범위로 아예
  선택 불가 처리하고, 확정된 날짜(●)와 아직 집계 중인 날짜(○)는 달력 팝업에 작은 점으로
  구분해 표시한다 — BaseWeb 달력이 각 날짜 셀 `aria-label`에 담는 영문 날짜 문자열
  부분일치로 CSS를 주입하는 방식이라 해시 클래스명에 의존하지 않아 Streamlit 버전이
  바뀌어도 잘 안 깨진다. 하단 콘텐츠는 채널별 수집 현황 + 카테고리별(프롭티어/경쟁사/
  시장동향) TOP3만 남기고, 관련 뉴스가 없는 카테고리는 섹션 자체를 숨긴다(예전엔 채널별
  주요뉴스 탭 + 카테고리별 TOP5 + 빈 섹션도 "해당 소식 없음" 캡션으로 계속 자리를 차지).

## Teams 웹훅 발송

`notify.py`가 오늘의 브리핑(채널별 집계 타일 + 자사/경쟁사/시장 TOP3, 브리핑 아카이브 화면과
동일한 구성)을 Microsoft Teams Webhook으로 Adaptive Card 형태로 보낸다. 설정 → 웹훅 탭에서
웹훅별 켜짐/꺼짐·테스트 발송·발송 시각(`data/webhook_schedule.json`, 예: `09:05`)을 관리하고,
"📤 지금 발송" 버튼으로 즉시 수동 발송도 가능하다. 발송 결과(대상/성공 수, 기사 건수)는
`db.webhook_send_logs`에 한 줄씩 남는다.

- **마크다운 이스케이프(2026-08-28)**: 기사 제목/요약은 원문 스크랩 텍스트를 그대로 옮겨오는데,
...
- **AI 요약(Gemini)**: 상세 카드의 핵심 요약(`gist`)은 PDF에 실제로 나오는 상위 5건에 한해서만
  ...
  근거 부족으로 요약하지 않고 기존 발췌 방식을 그대로 씀). 요약 목표 길이는 `summarizer.SUMMARY_LEN`
  (2026-08-26 기준 300자 — "문장을 중간에 끊지 마"라는 프롬프트 지시 때문에 실제로는 소프트
  타깃이라 기사에 따라 다소 넘길 수 있음)이며, AI 요약이 없을 때(실패 등) 원문 발췌 폴백 길이는
  `news_feed.PDF_CONTENT_PREVIEW_LEN`으로 같은 비율을 유지한다. 생성된 요약은 `mentions.summary`
...
- **지표 성격 질문은 함수 호출(automatic function calling)로 직접 집계해 답한다.**
  `agent_chat._STATS_TOOLS`에 29개 순수 함수를 그대로 등록해두면 google-genai SDK가 시그니처와
...
- **그라운딩 문구 오표시 수정(2026-08-28)**: 벡터 검색 컨텍스트가 없을 때(위에서 통계 질문이라
  ...
  **기본 수집 현황**
  - `get_channel_counts` — 특정 날짜의 채널별 수집 건수
  - `get_overview_stats` — 전체 지표 스냅샷
  - `get_brand_mention_count` — 특정 브랜드 누적 언급 건수
  - `get_policy_source_counts` — 기관별 정책 보도자료 건수
  - `get_briefing_highlights` — 특정 날짜 확정 브리핑의 실제 내용
  - `get_collection_health` — 채널별 최근 실행 시각/성공 여부
  - `get_vectorization_status` — 벡터화 진행 현황
  - `get_tracked_brands` — 현재 등록된 브랜드/키워드 목록
  - `get_collection_start_date` — 데이터 수집 시작일
  - `get_stale_channels` — 지정한 날짜에 수집이 0건인 채널
  - `get_policy_department_counts` — 정책 발표를 기관보다 세밀한 부서 단위로 집계

  **비교·추세**
  - `compare_brand_mentions` — 여러 브랜드 최근 N일 언급 비교
  - `get_top_mentioned_brands` — 최근 N일 언급 순위
  - `get_news_category_counts` / `get_policy_category_counts` — 카테고리별 건수
  - `compare_collection_periods` — 이번 기간 vs 직전 기간 수집량 비교
  - `get_brand_mention_trend` — 특정 브랜드 날짜별 언급 추이
  - `get_trending_brands` — 평소 대비 최근 언급 급증 브랜드
  - `get_brand_snapshot` — 특정 브랜드의 건수·추이·최근 기사를 한 번에(3개 도구 조합)

  **검색/조회**
  - `search_mentions` — 키워드/브랜드/기간으로 실제 기사 검색
  - `search_policy_events` — 키워드/부서/기간으로 정책 보도자료 검색(`search_mentions`의 정책 버전)
  - `get_top_viewed_policy_events` — 조회수 상위 정책 보도자료
  - `get_top_signal_news` — 오늘의 뉴스와 같은 점수 기준 상위 뉴스(최신순이 아니라 중요도순)
  - `get_brand_issues` — 부동산사 동향과 같은 이슈 클러스터링(같은 사건 기사 묶어서)

  **운영/비용**
  - `get_api_cost_summary` — Gemini API 호출량·추정 비용
  - `get_pdf_report_stats` — PDF 생성 횟수

  **온톨로지 그래프 쿼리** (아래 "온톨로지 기반 그래프 쿼리 도구 3종" 참고)
  - `graph_queries.category_alignment_counts` — 대응되는 뉴스/정책 카테고리 동시발생
  - `graph_queries.policy_event_mention_impact` — 정책 발표 전후 관련 뉴스 변화
  - `graph_queries.brand_role_category_breakdown` — 브랜드 role × 카테고리 교차집계

  </details>

  **2026-08-26 추가한 7개(쉬운 질문부터 어려운 질문까지 커버)**: `get_collection_start_date`/
  `get_policy_department_counts`(쉬움) → `search_policy_events`/`get_stale_channels`(중간)
  → `get_brand_snapshot`(중간~어려움) → `get_top_signal_news`/`get_brand_issues`(어려움,
  각각 `news_feed.build_news_items()`의 점수 랭킹과 `news_feed.build_issues()`의 이슈
  클러스터링을 챗봇에 처음으로 연결함 — 이전엔 화면에만 있고 AI AGENT는 전혀 못 씀).
  실제 코드베이스를 훑어 기존 22개와 안 겹치는 빈틈만 골랐고, 5개 질문(난이도별 1개씩)으로
  실제 Gemini 채팅까지 라이브 검증했다.
...
- **PDF 생성 콘텐츠 캐시 + single-flight 락** (`report_pdf.py`의 `get_or_generate_pdf_bytes()`) —
  ...
  카드별 최신성 문구(24시간 컷오프로 바뀌는 문구)처럼 렌더러가 읽는 모든 필드가 구조적으로
...
- **API 사용량 추정 비용** (`gemini_pricing.py`, 설정 → API 사용량 탭) — summarizer/agent_chat
  (`gemini-2.5-flash`, 입력 $0.30·출력 $2.50 / 100만 토큰)과 vectorizer(`gemini-embedding-001`,
  입력 $0.15 / 100만 토큰, 출력 개념 없음)는 실제 쓰는 모델이 달라 단가도 다르다 — 예전엔
  `views/settings.py`와 `agent_chat.py` 두 곳에 같은 단가 상수·계산식을 복붙해두고 있었는데,
  기능별 단가 분기가 생기며 드리프트 위험이 커져 이 모듈 하나로 합쳤다. 임베딩 API는
  `usage_metadata`를 안 줘서 실제 토큰 수를 모르는데, 실제 기사 원문으로 무료 `count_tokens`
  API를 표본 호출해 실측한 글자당 토큰 비율(1.4자 ≈ 1토큰)로 추정한다.
...
- **브리핑 아카이브를 `cached_db`로 전환(2026-08-26)** — `views/briefings.py`가 그동안
  `cached_db.py`에 이미 있던 60초 TTL 캐시를 안 쓰고 `db.py`를 직접 호출하고 있었다.
  특히 `get_distinct_mention_dates()`(어떤 날짜에 데이터가 있는지 찾는 쿼리, `date(collected_at)`에
  인덱스가 없어 mentions 전체를 스캔)가 달력에서 날짜를 하나 고를 때마다(=스크립트
  전체 재실행) 매번 다시 돌고 있었다 — 실측 6.6초(콜드) → 0.001초(캐시 적중). 확정된
  과거 날짜의 아카이브 내용은 한 번 쓰이면 절대 안 바뀌므로(`archive_pending_briefings`가
  `INSERT OR IGNORE`로 확정) 캐싱해도 안전하다.
- **`.sc-box`(검색/필터 감싸는 흰 카드) 제거 — Streamlit 함정** (`views/firms.py`/
  `views/policy_news.py`/`views/search.py`) — `st.markdown('<div class="sc-box">')`로 열고
  뒤에 `st.text_input` 등을 넣은 뒤 `st.markdown('</div>')`로 닫는 방식을 썼는데,
  Streamlit은 `st.markdown` 호출 하나하나를 독립된 HTML 조각으로 렌더링해서 **열린 `<div>`가
  그 조각 안에서 바로 자동으로 닫혀버려 뒤따르는 위젯을 전혀 감싸지 못한다** — 실제로는
  내용 없는 빈 흰 박스가 장식으로 떠 있고, 검색창/필터는 그 아래 스타일 없이 따로
  렌더링되고 있었다. 빈 박스를 제거하고 검색창+필터를 `st.columns`로 나란히 배치하는
  방식으로 교체했다 — Streamlit에서 여러 위젯을 하나의 스타일 박스로 감싸려면 `unsafe_allow_html`
  두 번으로 열고 닫는 방식은 쓰지 말 것(대신 `st.container(key=...)`가 만드는
  `st-key-*` 클래스를 CSS로 타겟하는 방식을 쓸 것).

- **배치 자동 수집 완료 시 캐시 즉시 무효화(2026-08-31)** (`collector.py`) — 전에는 배치
  수집(신규 게시물/정책/네이버뉴스/매경 API)이 끝나도 `cached_db`를 비우지 않아, F5를
  눌러도 최대 60초 동안 옛 데이터가 그대로 보였다. 사용자가 "F5 누르면 바로 신규 데이터가
  나와야 하는 것 아니냐"고 물어 확인 중 발견 — 이전엔 "감지 훅이 필요해 복잡도 대비 이득이
  적다"고 보류했던 부분인데, 실제로는 각 백그라운드 수집 작업의 `finally` 블록(수집 완료가
  확정되는 지점)이 이미 존재해 거기에 `cached_db.clear()` 호출 한 줄만 추가하면 되는
  간단한 문제였다. TDD로 4개 수집 경로 전부 테스트 후 적용.

**아직 다루지 않은 부분(의도적으로 보류):** 뉴스 검색/부동산사 동향 검색어 입력의 debounce.

## 신규 게시물 알림 종 (공통 헤더)

배치가 30분 등 주기로 돌아도 화면을 강제로 새로고침하지 않는다 — 기사를 읽던 중 스크롤이
튀거나 탭 선택이 풀리는 게 더 불편하다는 판단(2026-08-31). 대신 공통 헤더 우상단에 종(🔔)
아이콘을 두고, 새 데이터가 있다는 것만 조용히 알린 뒤 사용자가 원할 때 확인하게 한다.

- `notifications.py`(순수 함수) + `theme.notification_bell()` — 이미 쌓이고 있던
  `run_logs`/`policy_run_logs`(배치별 수집 이력)를 그대로 재사용해 "14:32 · 신규 게시물
  12건 수집 (네이버, 구글)" 같은 로그를 만든다. 새 테이블은 없다. `inserted=0`인 배치(전부
  중복이라 걸러진 경우)는 "신규"가 아니므로 알림 목록에서 제외한다.
- **읽음 처리는 세션(브라우저 탭) 단위** — 탭을 닫으면 초기화된다. `st.popover` 안 콘텐츠는
  열림/닫힘과 무관하게 매 rerun마다 그대로 실행되므로(파이썬 코드가 도는지는 프론트엔드가
  화면 표시만 가릴 뿐 실행 자체는 항상 됨) "팝오버를 열면 자동 읽음 처리"는 만들 수 없다 —
  안에 명시적 "모두 확인" 버튼을 둔다.
- `theme.floating_actions()`와 마찬가지로 `app.py`가 `st.navigation(...).run()` **이전에**
  호출해야 화면에 나타난다.
- **Streamlit 함정 — `[data-testid="stToolbar"]`는 Deploy 버튼만이 아니다**: 종 아이콘이
  Deploy 버튼과 겹치지 않게 `stToolbar`를 `visibility:hidden`으로 숨기려다가, 실제
  브라우저로 확인해보니 이 Streamlit 버전(1.56)에서는 상단 페이지 탭(`stTopNavLink`) 전체가
  `stToolbar`(`stAppToolbar`) 컨테이너 **안에** 중첩돼 있어서 탭 전체가 함께 사라지는 회귀가
  났다 — 겉보기엔 "Deploy 버튼 하나만 감추는 CSS"처럼 보이지만 실제 DOM 구조는 다르다.
  해당 CSS를 제거하고 종을 `z-index`로 위에 띄우는 방식으로 교체했다. 이 프로젝트에서
  Streamlit 헤더/툴바 영역을 다시 건드릴 일이 있으면, 반드시 실제 브라우저로 `stTopNavLink`의
  `computed style`(특히 `visibility`)까지 확인할 것 — DOM 구조는 버전마다 바뀔 수 있다.
...
- **크래시 내구성**: `db._connect()`가 매 연결마다 `PRAGMA synchronous=FULL`을 설정한다
  ...
  줄인다.
- **자동 백업**: `scheduler.py`의 `_tick_db_backup`이 `db.backup_database()`(SQLite 온라인
  백업 API — 다른 커넥션이 쓰기 중이어도 안전하게 스냅샷을 뜬다)를 호출해
  `data/db_backups/`에 저장하고, 오래된 것은 정리한다. 주기·보관 개수는
  `scheduler._DB_BACKUP_INTERVAL_MINUTES`/`db._BACKUP_KEEP`(2026-08-26 기준 하루
  1번 × 5개 = 최근 5일 커버) — 예전엔 20분마다 × 12개(4시간 커버)였는데, `news.db`가
  900MB 가까이 커지면서 20분마다 파일 전체를 복사하는 I/O 자체가 다른 요청들을 눈에
  띄게 느리게 만들어 주기를 늘렸다. 트레이드오프로 복구 시 최대 손실 가능 데이터가
  20분치 → 최대 24시간치로 늘어난다(무결성 검사는 여전히 10분마다 돌아 손상 자체는
  빠르게 잡아낸다).
...
- **배포 시 로컬 파일이 서버 걸 덮어쓰지 않도록**: `views/settings.py`의
  ...
  덮어쓰고 있었다.
- **DB 용량 절감(2026-08-26)**: `mentions.embedding`(원본 임베딩, "벡터 데이터" 섹션 참고)이
  JSON 텍스트로 저장돼 있어 숫자 하나당 평균 13~14바이트를 먹었다 — 이 컬럼 하나가
  DB 전체 용량의 70%(599MB/860MB)를 차지하고 있었고, 위 백업 I/O 부담의 근본 원인이기도
  했다. `vectorizer._pack_embedding()`으로 struct-packed float32 바이너리(4바이트/개)로
  바꾸면서 599MB → 175MB로 줄었고, 기존 14,948건도 전량 마이그레이션 + `VACUUM`으로
  DB 파일 자체가 904MB → 428MB가 됐다. 실제로 임베딩 일부를 비운 뒤 압축 백업 파일로
  복구해 바이트 단위로 원본과 일치하는지까지 검증했다.
- **측정된 비용**: 무결성 검사·백업 모두 사용자 수와 무관하게 파일 크기에만 비례하는
  작업이라 동시 접속이 늘어도 부담이 커지지 않는다. 디스크는 백업 5개 × DB 크기만큼
  쓴다(현재 기준 약 2GB 안팎으로 안정화 예정 — DB가 계속 자라므로 완전히 고정된 값은
  아니다).
- **정직한 한계**: 이건 손상 "원인"을 없애는 게 아니라 "피해"를 최소화하는 것이다 —
  가장 최근 백업 시점 이후의 데이터(최대 24시간치)는 사고마다 유실될 수 있다. 근본
...
- **벡터 데이터**: `vectorizer.py`가 Gemini 임베딩 모델(`gemini-embedding-001`, 3072차원)로
  ...
  컬럼(struct-packed float32 바이너리, 원본 보관용 — 컬럼 선언은 `TEXT`지만 SQLite가
  동적 타이핑이라 BLOB 값을 그대로 저장할 수 있다. 2026-08-26 이전엔 JSON 텍스트였으나
  용량 절감을 위해 바꿨다, "DB 안정성" 섹션 참고. 구버전 JSON 텍스트도 하위호환으로 계속
  읽을 수 있다)과 `sqlite-vec` 색인(`mention_vectors`/`policy_vectors`, vec0
...
- **벡터 임베딩 백업/복구** (설정 → 벡터 데이터 탭 하단): `mention_vectors`/`policy_vectors`(vec0
  색인)는 `mentions.embedding`/`policy_events.embedding`(원본)으로부터 언제든 재생성
  가능한 파생 데이터라 백업 대상에서 빼고, url을 키로 삼은 embedding 값만 JSON 파일로
  내보낸다(`vectorizer.export_vector_backup`) — id가 아니라 url을 키로 쓰는 이유는 DB 복구
  후 id가 바뀌어도 url은 유지되기 때문. embedding이 이제 바이너리라 JSON에 그대로 못 담아,
  `vectorizer._to_transport()`가 base64 문자열로 바꿔서 파일에 담고 복구 시
  `decode_vector_backup_rows()`가 다시 바이트로 되돌린다. 복구는 url이 일치하고 아직
  embedding이 비어있는 행에만 채워 넣고(이미 값이 있으면 덮어쓰지 않음)
  `vectorizer.sync_vector_index()`로 색인도 함께 채운다 — DB 손상 등으로 색인이 날아갔을 때
  Gemini 임베딩 API를 다시 호출하지 않고도 되돌릴 수 있다. 복구 버튼을 누르면 `st.status()`로
  뉴스 복구 → 정책 복구 → 색인 재생성 단계가 실시간으로 표시된다. 건마다 새 커넥션을 여는
  대신 커넥션 하나로 전체를 처리해 수천 건 규모도 몇 초 안에 끝난다. 임베딩 일부를 비운
  뒤 이 경로로 복구해 바이트 단위 일치까지 실측 검증했다.
...
설정 화면의 "🚀 배포" 탭은 `DEPLOY_HOST`가 설정된 환경(로컬 개발 머신)에서만 나타난다 —
아래 "배포된 서버는 자기 자신을 배포 대상으로 모른다" 항목 참고.
...
**배포된 서버는 자기 자신을 배포 대상으로 모른다(2026-08-26)**: 예전엔 배포할 때 로컬
`.env`를 통째로 서버에 올려서, 배포된 서버 자신도 `DEPLOY_HOST`(자기 자신) SSH 접속 정보를
그대로 갖게 됐다 — 그 서버의 설정 화면에서도 "배포" 탭이 똑같이 살아나, 관리자가 실수로
그 서버 자신에게서 "서버에 배포"를 누르면 자기 자신을 재배포하는 상황이 가능했다(실제로
DB 손상 사고 중 하나의 트리거였다). `views/settings.py`의 `_filtered_env_content()`가 업로드
직전 `.env`에서 `DEPLOY_` 접두사가 붙은 줄을 전부 걸러내고, `render()`는 `DEPLOY_HOST`가
없으면 "🚀 배포" 탭 자체를 만들지 않는다 — 서버 쪽에는 다음 배포부터 반영된다(이미 올라가
있는 예전 `.env`는 그 다음 배포 때 덮어써지면서 자동으로 정리됨).

**배포 실패 "Socket is closed" / "EOF during negotiation"는 코드 문제가 아니라 서버 계정
비밀번호 만료(2026-08-31 실측)**: SSH 연결 자체는 성공("✅ SSH 연결 완료"까지 로그가 찍힘)하는데
그 다음 단계(SFTP 오픈 등)에서 이 오류로 실패한다면, `DEPLOY_USER` 리눅스 계정의 비밀번호가
만료된 것이다. `paramiko`로 직접 `exec_command`를 실행해보면 stderr에 `WARNING: Your password
has expired. Password change required but no TTY available.`가 찍히는데, PAM이 "비밀번호부터
바꿔야 하는데 대화형 터미널이 없어 바꿀 수 없다"며 그 시점에 SSH 트랜스포트 자체를 끊어버려서,
이후 새 채널(SFTP 등)을 열려는 시도가 전부 "Socket is closed"로 실패한다. 해결: 실제 터미널로
SSH 접속해 비밀번호를 갱신하고 `.env`의 `DEPLOY_PASS`도 함께 갱신할 것. 재발을 막으려면 서버
관리자 권한으로 `sudo chage -M -1 <DEPLOY_USER>`로 비밀번호 만료 정책 자체를 없애는 걸 권장.
...
gemini_pricing.py   Gemini API 추정 비용 계산 공용 로직 (기능별 단가 분기 + 임베딩 토큰 글자수 기반 추정)
...
## 인수인계 문서 · 시스템 청사진 (HTML, 2026-08-31 ~ 09-02)

이 프로젝트 전용 온보딩 문서 2종 + 시스템 청사진 1종을 만들어뒀다(다른 로컬 산출물처럼 이
저장소에는 포함하지 않음). 팀의 다른 프로젝트(`lex_agent`, `AiAxRadar` 등) 문서와 같은 디자인
시스템(같은 CSS/JS)을 재사용해, 목차 스크롤스파이·진행률 추적기(localStorage)·"바이브 코딩으로
말하기" 상자 등 인터랙션이 동일하게 동작한다. 이 시점부터 사내 문서·대화에서는 이 프로젝트를
`hana_p` 대신 **`proptier_news`**로 지칭한다(저장소 폴더명·git 원격 이름은 그대로).

- **`C:\Users\USER\Desktop\Onbording\proptier_news Onbording (완).html`**(원래 이름
  `hana_p 인수인계 문서.html`, 사용자가 폴더/파일명을 변경함) — 기존 코드를 그대로 쓰는 표준
  온보딩. Part I(신규 PC에서 git clone부터 배포까지 9단계) + Part II(9개 모듈의 "왜 이렇게
  만들었나" 개발 과정 + 설계 스펙 + 화면 목업) + "직접 기능 추가해보기"(새 크롤러/AI AGENT
  도구/새 화면을 바이브 코딩 프롬프트로 확장하는 예시 3개). masthead 직후에 "한눈에 보는
  전체 기능"(`#features`, 화면 8종 표)과 "AI는 어디에, 어떻게 쓰이나"(`#ai-guide`, 임베딩·
  RAG·함수호출·그라운딩 쉬운 설명) 섹션을 추가로 뒀다. 09-02에 표기상 "hana_p"를
  "proptier_news"로 교체했으나(제목·헤더 등), git clone 폴더명·venv 이름·쿠키명
  (`hana_p_uid`)처럼 실제 코드/명령어를 그대로 반영하는 부분은 정확성을 위해 남겨뒀다.
- **`C:\Users\USER\Desktop\인수인계문서\hana_p 설계 기반 개발 가이드 (소스코드 없는 버전).html`**
  — 이 저장소의 소스코드를 열람·클론하지 않고도 같은 서비스를 처음부터 새로 설계·구축할 수
  있게 만든 버전. Part II가 "무엇을, 왜 이렇게"만 규정한 설계 명세(데이터 모델·수집·캐싱·
  화면 8종·AI AGENT·PDF·웹훅·운영 안정성·배포)이고, Part III에 그 설계를 실제로 구현하는
  10단계 빌드 순서(각 단계마다 바이브 코딩 프롬프트)를 뒀다. 이 문서 자체를 Claude Code에게
  통째로 읽혀 "이 설계대로 만들어줘"라고 하는 게 시작점이다. "전체 기능 한눈에 보기"
  (`#features-overview`)와 "AI 기술 이해하기"(`#ai-concepts`, 개념 5가지 쉬운 설명) 섹션을
  추가로 뒀다.
- **`C:\Users\USER\Desktop\workflow\proptier_news flow.html`** — 팀의 다른 프로젝트
  (`AiAxRadar`, `LexAgent`) "시스템 청사진" 문서와 같은 포맷(hero 통계행 + SVG 전체흐름도 +
  AI 파이프라인 확대 다이어그램 + "왜 이렇게" 컬럼이 있는 AI 기술요소표 + 프롬프트 가드레일 +
  설계 결정 + 실패해도 안 멈추는 지점 + 동시 사용자 대비(관찰된 문제·원인·조치) + 기술 스택 +
  구성요소 참조표)으로 만든 아키텍처 문서 — 온보딩 문서가 "어떻게 쓰는지"라면 이 문서는 "왜
  이렇게 만들었는지"를 실제 코드·DB 실측값(테이블 행수, 벡터 색인율, distance 임계값 등)으로
  채운다. 두 개 세션이 같은 파일을 동시에 편집하는 사고가 있어 수동으로 병합·정리했다.

두 온보딩 문서는 2026-08-31/09-01에 "비개발자가 봐도 이해할 수 있게" + "AI 기술/전체 기능
설명"을 보강했다 — 기존 `.callout--easy`/`.spec`/`.table-wrap` 디자인 클래스만 재사용했고
신규 CSS는 추가하지 않았다.
