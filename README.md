...
## 인수인계 문서 (HTML, 2026-08-31, 09-01 보강)

이 프로젝트 전용 온보딩 문서 2종을 만들어뒀다(다른 로컬 산출물처럼 이 저장소에는 포함하지
않음). 팀의 다른 프로젝트(`lex_agent`) 온보딩 문서와 같은 디자인 시스템(같은 CSS/JS)을
재사용해, 목차 스크롤스파이·진행률 추적기(localStorage)·"바이브 코딩으로 말하기" 상자 등
인터랙션이 동일하게 동작한다.

- **`C:\Users\USER\Desktop\Onbording\proptier_news Onbording.html`**(원래 이름
  ...
  RAG·함수호출·그라운딩 쉬운 설명) 섹션을 추가로 뒀다.
...
두 문서 모두 2026-09-01에 "비개발자가 봐도 이해할 수 있게" + "AI 기술/전체 기능 설명"을
보강했다 — 기존 `.callout--easy`/`.spec`/`.table-wrap` 디자인 클래스만 재사용했고 신규 CSS는
추가하지 않았다.
