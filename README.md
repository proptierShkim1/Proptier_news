...
## 인수인계 문서 (HTML, 2026-08-31)

`C:\Users\USER\Desktop\인수인계문서\`에 이 프로젝트 전용 온보딩 문서 2종을 만들어뒀다(다른
로컬 산출물처럼 이 저장소에는 포함하지 않음). 팀의 다른 프로젝트(`lex_agent`) 온보딩 문서와
같은 디자인 시스템(같은 CSS/JS)을 재사용해, 목차 스크롤스파이·진행률 추적기(localStorage)·
"바이브 코딩으로 말하기" 상자 등 인터랙션이 동일하게 동작한다.

- **`hana_p 인수인계 문서.html`** — 기존 코드를 그대로 쓰는 표준 온보딩. Part I(신규 PC에서
  git clone부터 배포까지 9단계) + Part II(9개 모듈의 "왜 이렇게 만들었나" 개발 과정 + 설계
  스펙 + 화면 목업) + "직접 기능 추가해보기"(새 크롤러/AI AGENT 도구/새 화면을 바이브 코딩
  프롬프트로 확장하는 예시 3개).
- **`hana_p 설계 기반 개발 가이드 (소스코드 없는 버전).html`** — 이 저장소의 소스코드를 열람·
  클론하지 않고도 같은 서비스를 처음부터 새로 설계·구축할 수 있게 만든 버전. Part II가
  "무엇을, 왜 이렇게"만 규정한 설계 명세(데이터 모델·수집·캐싱·화면 8종·AI AGENT·PDF·웹훅·
  운영 안정성·배포)이고, Part III에 그 설계를 실제로 구현하는 10단계 빌드 순서(각 단계마다
  바이브 코딩 프롬프트)를 뒀다. 이 문서 자체를 Claude Code에게 통째로 읽혀 "이 설계대로
  만들어줘"라고 하는 게 시작점이다.
